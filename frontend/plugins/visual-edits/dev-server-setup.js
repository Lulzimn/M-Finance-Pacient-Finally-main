...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    // CORS origin validation (locked down by default)
    const isAllowedOrigin = (origin) => {
      ...
      // Optional single custom origin via env var ALLOWED_ORIGIN
    ...
    // ✅ Health check (no auth) — can be disabled by setting ENABLE_PING=false
    ...
    // ✅ Protected file editing endpoint with AST processing (opt-in only)
    if (process.env.ENABLE_VISUAL_EDITS === "true") devServer.app.post("/edit-file", (req, res) => {
